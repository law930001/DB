...
class ImageDataset(data.Dataset, Configurable):
        ...
        for i in range(len(self.data_dir)):
            ...
            if self.is_training:
                if 'SynthText' in self.data_list[i]:
                    ...
                    gt_path=[self.data_dir[i]+'/gt/'+timg.strip().split('/')[1].replace('.jpg','')+'.txt' for timg in image_list]
                else:
                ...
                elif 'T-Brain' in self.data_list[i]:
    ...
    def load_ann(self):
        ...
        for gt in self.gt_paths:
            ...
            lines = []
            if 'SynthText' in gt:
                gt = gt.replace("._", '') # add by me
            reader = open(gt, 'r').readlines()
            if 'SynthText' in gt:
                for k, line in enumerate(reader):
                    if k == 0: # pass the first data
                        continue
                    item = {}
    ...
    def __getitem__(self, index, retry=0):
        ...
        if 'SynthText' in image_path:
            image_path = image_path.replace("._", '') # add by me
